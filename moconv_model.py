...
from .base_module import get_norm, ConvNormAct ,LayerNorm, get_act, h_sigmoid
...
class MoConvBlock(nn.Module):
    def __init__(self,
        ...
        # self.attn = iRMB(in_channels,in_channels)
        # self.attn0 = iRMB(in_channels//2, in_channels//2)
        # self.attn = ELA(in_channels)
        self.attn0 = CA(in_channels // 2)
        # self.attn0 = ELA(in_channels // 2)
        # self.attn0 = CA_atten(in_channels // 2)
        self.attn1 = ECA(in_channels // 2)
        # self.attn1 = SE(in_channels//2, ratio=2)
        # self.attn = CA(in_channels)
        # self.attn = NonLocalBlock(in_channels=in_channels,inter_channels=in_channels//2)
        ...
        self.irmb = InvertedResidual( in_channels=in_channels, out_channels=in_channels, stride=stride, expand_ratio = mv_expand_ratio )
        # self.mlp = MLP(in_featrues=in_channels, ratio=4)
        # self.group = group
    def forward(self, x):
...
def MoConv_N_small(num_classes: int = 1000):
...
def MoConv_T_small(num_classes: int = 1000):
    # pretrain weight link
